...
beifen:addEffect(fk.AfterCardsMove, {
  ...
  trigger_times = function (self, event, target, player, data)
    if player:hasSkill(beifen.name) and player:getMark("yyfy_shuangjia") ~= 0 then
      local cards = {}
      for _, move in ipairs(data) do
        if move.from == player then
          for _, info in ipairs(move.moveInfo) do
            if info.fromArea == Card.PlayerHand and table.contains(player:getTableMark("yyfy_shuangjia"), info.cardId) then
  ...
  on_use = function(self, event, target, player, data)
    ...
    for _, id in ipairs(player:getCardIds("h")) do
      ...
      if card:getMark("@@yyfy_shuangjia") > 0 then
