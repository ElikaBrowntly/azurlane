...
Fk:loadTranslationTable {
  ["lan__buchen"] = "不臣",
  [":lan__buchen"] = "隐匿技，当你于其他角色的回合登场后，你可令其本回合手牌上限减至0。" ..
      "此回合结束时，你获得其弃牌阶段内弃置的牌。",

  ["@@lan__buchen-turn"] = "不臣",
  ["#lan__buchen-invoke"] = "不臣：你可以令 %dest 本回合手牌上限减至0",

  ["$lan__buchen1"] = "螟蛉之光，安敢同日月争辉？",
  ["$lan__buchen2"] = "巍巍隐帝，岂可为臣？",
}
...
buchen:addEffect(U.GeneralAppeared, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    if room:askToSkillInvoke(player, {
          ...
          prompt = "#lan__buchen-invoke::" .. room.current.id,
        }) then
  ...
  on_use = function(self, event, target, player, data)
    ...
    player.room:addPlayerMark(to[1], "@@lan__buchen-turn")
  end,
})

buchen:addEffect("maxcards", {
  fixed_func = function(self, player)
    if player:getMark("@@lan__buchen-turn") > 0 then
      return 0
    end
  end,
})

buchen:addEffect(fk.TurnEnd, {
  anim_type = "drawcard",
  is_delay_effect = true,
  can_trigger = function(self, event, target, player, _)
    if player.dead or player:usedSkillTimes(buchen.name) == 0 then return false end
    local room = player.room
    local discard_ids = {}
  ...
  on_use = function(self, event, _, player, _)
    ...
    player.room:obtainCard(player, cards, true, fk.ReasonJustMove, player, buchen.name)
    -- fk.ReasonPrey 是获得其他玩家的牌，prey是掠夺、猎物的意思，这里从弃牌堆获得所以是JustMove
    -- 顺便，按此道理，另一名角色收回自己装备区里的牌，也应该是fk.ReasonJustMove
  end,
})

buchen:addAI(Fk.Ltk.AI.newInvokeStrategy{
