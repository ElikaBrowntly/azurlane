...
Fk:loadTranslationTable{
  ["yyfy_wangxi"] = "忘隙",
  [":yyfy_wangxi"] = "当你对其他角色造成1点伤害后，或当你受到其他角色造成的1点伤害后，若其存活，你可以摸两张牌，然后将一张牌交给该角色。",

  ["#yyfy_wangxi-invoke"] = "忘隙：是否对 %dest 发动“忘隙”，摸两张牌并交给其一张牌",
  ["#yyfy_wangxi-give"] = "忘隙：交给 %dest 一张牌",

  ["$yyfy_wangxi1"] = "小隙沉舟，同心方可戮力。",
  ["$yyfy_wangxi2"] = "为天下苍生，自当化解私怨。",
}
...
wangxi:addEffect(fk.Damage, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    if room:askToSkillInvoke(player, {
      ...
      prompt = "#yyfy_wangxi-invoke::"..data.to.id,
    }) then
  ...
  on_use = function(self, event, target, player, data)
    ...
    local card = room:askToCards(player, {
      ...
      include_equip = true,
      skill_name = wangxi.name,
      prompt = "#yyfy_wangxi-give::"..data.to.id,
      cancelable = false,
    })
    if #card > 0 then
...
wangxi:addEffect(fk.Damaged, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    if room:askToSkillInvoke(player, {
      skill_name = wangxi.name,
      prompt = "#yyfy_wangxi-invoke::"..data.from.id,
    }) then
  ...
  on_use = function(self, event, target, player, data)
    ...
    local card = room:askToCards(player, {
      ...
      include_equip = true,
      skill_name = wangxi.name,
      prompt = "#yyfy_wangxi-give::"..data.from.id,
      cancelable = false,
    })
    if #card > 0 then
