...
Fk:loadTranslationTable{
  ...
  "<br><font color='#93DB70'>山猪</font>：获得此技能时，令一名其他角色失去所有体力"..
  "<br><font color='green'>少年</font>：其他角色受到伤害后，或出牌阶段限一次你可以选择一名其他角色，你可以赐予其一个其他化身(未实装)"..
  "<br><font color='cyan'>凤凰</font>：其他角色至你的距离恒为10，你不是其他角色使用牌的合法目标(未实装)"..
  "<br><font color='blue'>牡羊</font>：限定技，你死亡时改为修整一回合(未实装)"..
...
quanneng:addEffect(fk.TurnStart, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    for _, skill_name in ipairs(chosen_skills) do
      room:handleAddLoseSkills(player, skill_name, "yyfy_quanneng")
    end
    -- 记录选择的技能数量
...
quanneng:addEffect(fk.TurnEnd, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    -- 失去体力
    if skill_count > 1 and player:isAlive() then
      room:loseHp(player, skill_count - 1, "yyfy_quanneng")
    end
