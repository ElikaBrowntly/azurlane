...
konggou:addAcquireEffect(function(self, player, is_start)
  ...
  for _, name in ipairs(hand_change) do
    room:addTableMarkIfNeed(player, "yyfy_konggou-hand", name)
  end
end
)
-- 回合内用视为技view_as，印各种牌
...
konggou:addEffect("viewas", {
  ...
  before_use = function(self, player, use)
    if player.id < 0 then return "Cancel" end
    local name = self.interaction.data
    ...
    local case = 0
    if table.contains(pure_hp, name) then
      case = 1
  ...
  enabled_at_play = function(self, player)
    return player.hp ~= 1 or #player:getCardIds(Player.Hand) ~= 1 and player.id > 0
  end
})
-- 求桃时印桃
konggou:addEffect(fk.AskForCardUse, {
  can_trigger = function(self, event, target, player, data)
    ...
    if table.contains(player:getTableMark("yyfy_konggou-hp"), "analeptic") and
        Exppattern:Parse(data.pattern):matchExp("analeptic") then
      case = case + 2
    end
...
konggou:addEffect(fk.TurnEnd, {
  can_refresh = function(self, event, target, player, data)
    return player and player:hasSkill(self.name, true) and player.id > 0 and
