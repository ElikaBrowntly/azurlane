...
Fk:loadTranslationTable{
  ["lan__jifa"] = "冀筏",
  [":lan__jifa"] = "锁定技，当你进入濒死状态时，你减X点体力上限（X为你上次发动〖赴曦〗时选择的项数），"..
  "选择此次“退幻”时保留〖煌烛〗或〖离渊〗，然后<a href='os_tuihuan_caoang'>“退幻”</a>并将体力回复至体力上限。",

  ["os_tuihuan_caoang"] = "变身为表形态：<br>"..
  "<b>炽灰</b>：其他角色的回合开始时，你可以选择：1.弃置其区域里的一张牌；"..
  "2.将牌堆里的一张对应副类别的牌置入其装备区。若如此做，你可以先选择是否失去1点体力，",
  "然后可以摸X张牌（X为你已损失的体力值）。"..
  "<b>赴曦</b>：持恒技，当你进入濒死状态时，或你的装备栏均被废除后，你可以选择一至两项，然后"..
  "<a href='os_ruhuan_caoang'>“入幻”</a>并将体力回复至体力上限：<br>1.此次“入幻”时保留〖炽灰〗；"..
  "<br>2.将手牌摸至X张（X为你的体力上限）；<br>3.恢复所有被废除的装备栏。",

  ["#lan__jifa-choice"] = "冀筏：选择本次“退幻”时保留的技能",

  ["$lan__jifa1"] = "往昔如水旧新事，身赴黄夕又经年。",
  ["$lan__jifa2"] = "淯水川流如斯，难尽昔日扰攘。",
}
...
jifa:addEffect(fk.EnterDying, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    local x = player:getMark("lan__fuxi")
    if x > 0 then
    ...
    if #choices == 2 then
      local choice = room:askToChoice(player, {
        ...
        prompt = "#lan__jifa-choice",
        detailed = true,
      })
      table.removeOne(choices, choice)
      skills = "-" .. choices[1] .. "|"
    end
    room:handleAddLoseSkills(player, skills .. "-lan__jifa|lan__chihui|lan__fuxi", nil, true, false)
  end,
})
